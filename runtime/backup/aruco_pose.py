...
def detect_apriltags(image_path, target_ids=None):
    ...
    # ===== 预处理在外 × 尺度在内, 共 9×2 = 18 次尝试 =====
    def _gamma(g, gamma):
        t = np.clip(g.astype(np.float64) / 255.0, 0, 1) ** gamma
    ...
    preprocessors = [
        ("原图",            lambda g: g),
        ("CLAHE",           lambda g: _clahe.apply(g)),
        ("直方图均衡",      lambda g: cv2.equalizeHist(g)),
        ("自适应阈值-高斯", lambda g: cv2.adaptiveThreshold(
                                 g, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                 cv2.THRESH_BINARY, 21, 10)),
        ("自适应阈值-均值", lambda g: cv2.adaptiveThreshold(
                                 g, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                                 cv2.THRESH_BINARY, 21, 10)),
        ("伽马提亮γ0.5",    lambda g: _gamma(g, 0.5)),
        ("伽马压暗γ2.0",    lambda g: _gamma(g, 2.0)),
        ("高斯3x3",         lambda g: cv2.GaussianBlur(g, (3, 3), 0)),
        ("中值3x3",         lambda g: cv2.medianBlur(g, 3)),
    ]

    scales = [1.0, 2.0]   # 原图 / 2×放大
    ...
    def _has_all_targets(ids_flat, tids):
        """检测到的 ID 是否包含全部目标 ID。"""
        if tids is None:
            return True
        return all(int(tid) in ids_flat for tid in tids)

    for name, prep_fn in preprocessors:          # 预处理在外
        for scale in scales:                     # 尺度在内
            try:
                g_proc = prep_fn(gray)
            except Exception:
                continue
            ...
            cs_t, ids_t, rej_t = detector.detectMarkers(g_try)
            n_rej = 0 if rej_t is None else len(rej_t)
            n_rej_max = max(n_rej_max, n_rej)
            if ids_t is not None:
                ids_t_flat = ids_t.flatten()
                if not _has_all_targets(ids_t_flat, target_ids):
                    # 检测到标记但缺目标 ID → 继续试下一个方案
                    continue
                corners = ([c / scale for c in cs_t] if scale != 1.0 else cs_t)
                ids = ids_t
                rejected = rej_t
                used_desc = f"{name}+{scale}×"
                break
    ...
    if corners is None:
        raise RuntimeError(
            f"未检测到包含全部目标标记的 AprilTag (共尝试 "
            f"{len(preprocessors) * len(scales)} 种组合, rejected 候选最多 {n_rej_max} 个)。"
            f"请确认图片中存在 AprilTag 36h11 字典的标记且画面清晰无遮挡。")

    if used_desc != "原图+1.0×":
        print(f"[提示] 通过预处理方案 [{used_desc}] 识别成功")
