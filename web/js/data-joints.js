// data-joints.js
// 关节角度轴数据管理页面：读取、到位、更新、删除
// 只处理 joints 数据，不处理坐标和地图点位
        ...
        <div class="djc-actionbar">
            <div class="djc-left-info">
                ...
                <span v-if="selectedItem" class="djc-sel-info">
                    选中: 关节 · {{ selectedItem.type }} · {{ selectedItem.name }}
                </span>
            </div>
    ...
    methods: {
        ...
        onDataResp(data) {
            if (data && data.command === 'response' && Array.isArray(data.data)) {
                // 只保留关节数据
                this.items = data.data.filter(it => it.category === 'joints');
                this.selectedIdx = -1;
                console.log('[关节数据] 收到', this.items.length, '条数据');
            }
        },
        ...
        confirmGoTo() {
            ...
            this.showConfirm(
                '确认到位',
                `确定要让机器人运动到关节「${item.type}/${item.name}」吗？请确保周围环境安全。`,
                '▶',
                'djc-icon-go',
                'djc-btn-go',
                () => this.doGoTo()
        ...
        doGoTo() {
            ...
            console.log('[关节数据] 到位', item.type, item.name);
        },
        confirmUpdate() {
            const item = this.selectedItem;
            if (!item) return;
            this.showConfirm(
                '确认更新',
                `确定要用当前机器人姿态更新关节「${item.type}/${item.name}」吗？原有数据将被覆盖。`,
                '↻',
                'djc-icon-update',
                'djc-btn-update',
                () => this.doUpdate()
        ...
        doUpdate() {
            ...
            console.log('[关节数据] 更新', item.type, item.name, Object.keys(dataToSend).length, '项');
        },
        confirmDelete() {
            const item = this.selectedItem;
            if (!item) return;
            this.showConfirm(
                '确认删除',
                `确定要删除关节「${item.type}/${item.name}」吗？此操作不可恢复。`,
                '✕',
                'djc-icon-delete',
                'djc-btn-delete',
                () => this.doDelete()
        ...
        doDelete() {
            ...
            this.selectedIdx = -1;
            console.log('[关节数据] 删除', item.type, item.name);
        }
    },
    mounted() {
